...
import { Sparkles, Check, Zap, Building2, Users, Mail } from "lucide-react";
const plans = [{
  name: "Grundskola",
  description: "För enskilda skolor som vill komma igång",
  price: "Gratis",
  priceNote: "under pilotperioden",
  features: ["Obegränsade lektionsinspelningar", "AI-transkription och sammanfattning", "Upp till 50 elever", "5 lärarklasser", "E-postsupport"],
  cta: "Kontakta oss",
  ...
  features: ["Allt i Grundskola", "Obegränsat antal elever", "Obegränsat antal lärare", "Prioriterad support", "Anpassad onboarding", "Avancerad statistik"],
  cta: "Kontakta oss",
  ...
  features: ["Allt i Skola Pro", "Fleranvändarhantering", "SSO-integration", "Dedikerad kontaktperson", "SLA-avtal", "Anpassade integrationer"],
  cta: "Boka demo",
  ...
  return <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      <section className="py-12 px-6 relative z-10">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => <Card key={index} className={`glass-panel relative overflow-hidden transition-all duration-300 hover:scale-[1.02] ${plan.highlighted ? "border-primary/50 ring-2 ring-primary/20" : "border-white/10"}`}>
              {plan.highlighted && <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-accent to-secondary" />}
              <CardHeader className="text-center pt-10 pb-6">
                <div className={`w-16 h-16 rounded-2xl mx-auto mb-6 flex items-center justify-center ${plan.highlighted ? "bg-primary/20" : "bg-white/5"}`}>
                  {index === 0 ? <Users className={`w-8 h-8 ${plan.highlighted ? "text-primary" : "text-muted-foreground"}`} /> : index === 1 ? <Zap className="w-8 h-8 text-primary" /> : <Building2 className={`w-8 h-8 ${plan.highlighted ? "text-primary" : "text-muted-foreground"}`} />}
                </div>
              ...
              <CardContent className="px-8 pb-10 space-y-8">
                ...
                <Button className={`w-full h-14 rounded-xl font-black text-lg transition-all ${plan.highlighted ? "bg-primary text-white hover:bg-primary/90 glow-primary" : "bg-white/5 text-white hover:bg-white/10 border border-white/10"}`} asChild>
