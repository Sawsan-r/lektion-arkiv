...
const TeacherLessonView = () => {
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      ...
      setLesson({ ...lesson, summary: editedSummary });
      setIsEditingSummary(false);
      toast({ title: "Sammanfattning sparad!" });
    } catch (error) {
      console.error("Error saving summary:", error);
      toast({ title: "Fel", description: "Kunde inte spara sammanfattningen", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSaveTranscription = async () => {
    ...
    try {
      ...
      setLesson({ ...lesson, transcription: editedTranscription });
      setIsEditingTranscription(false);
      toast({ title: "Transkription sparad!" });
    } catch (error) {
      console.error("Error saving transcription:", error);
      toast({ title: "Fel", description: "Kunde inte spara transkriptionen", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          <button
            onClick={() => navigate(`/teacher/class/${lesson.class_id}`)}
          ...
          <div className="flex items-center gap-2 text-primary">
            ...
            <span className="text-sm font-black uppercase tracking-[0.2em]">Lärarvy</span>
          </div>
                ...
                <div className="space-y-8">
                    ...
                    {isEditingSummary ? (
                      <>
                        <Button
                          ...
                          onClick={() => {
                            setIsEditingSummary(false);
                            setEditedSummary(lesson.summary || "");
                          }}
                          className="h-12 rounded-xl glass-button border-white/10 gap-2 font-bold"
                          disabled={isSaving}
                        >
                          <X className="w-5 h-5" />
                          Avbryt
                        ...
                        <Button
                          ...
                          className="h-12 rounded-xl bg-primary text-white gap-2 font-bold"
                          disabled={isSaving}
                        >
                  ...
                  {isEditingSummary ? (
                    <Textarea
                      ...
                      className="min-h-[400px] glass-input rounded-xl border-white/10 text-base leading-relaxed"
                      placeholder="Skriv sammanfattningen här..."
                    />
                  ) : (
                ...
                <div className="space-y-8">
                    ...
                    {isEditingTranscription ? (
                      <>
                        <Button
                          ...
                          onClick={() => {
                            setIsEditingTranscription(false);
                            setEditedTranscription(lesson.transcription || "");
                          }}
                          className="h-12 rounded-xl glass-button border-white/10 gap-2 font-bold"
                          disabled={isSaving}
                        >
                  ...
                  {isEditingTranscription ? (
                    <Textarea
                      ...
                      className="min-h-[400px] glass-input rounded-xl border-white/10 text-base leading-relaxed"
                      placeholder="Skriv transkriptionen här..."
                    />
                  ) : (
